...
break;
			...
			case 31:
			{
				...
				wptr = fopen("stats.txt","w");
					...
					/* Car HP and armor HP equal total player HP; then generate a random number of cops from 1  t0 10 and multiply that into cop HP. 
					 * IF the player HP is higher than all the Cops HP convined then the player will win ELSE they lose.
					 * Finally, the the results for the run would be documented and written.*/

                                        puts("\nyou open the door and find..");
                                        puts("Oscar's room...");
                                        puts("**Oscar approaches you**'Hey! You're here right on time!");
                                        puts("We're about to commit a bank heist and its up to you to select our vehicle and armor in order to make a swift escape");
                                        puts("1. You can select a car from my car collection?");
                                        puts("2. You can select armor from my armor collection?");
					puts("3. Press 3 when you are ready to go");
                                        puts("4. Press 4 when you want so to another room");
                                        puts("Press 99 to quit at any moment");
                                        scanf("%d", &choice);
					...
					{ 
                                        	...
                                        	if (choice == 1)
						{
							...
							carselected = bHP + sHP + aHP;


                                                	puts("I have a:\n1. Lamborghini\n2. Ferrari\n3. Mercedez\n");
                                               	 	puts("Pick a number don't be shy...");
                                                	scanf("%d",&choice);
						...
						else if (choice == 2)
						{
							...
							armorselected = lHP + mHP + hHP;

                                               		puts("1. Light\n2. Medium\n3. Heavy\n");
                                               		puts("You're armor is the most important thing as cops are tired always loosing to our speedy cars and they have become more aggresive.");
                                                	puts("If you armor goes then so will the car, but maybe if you're light and quick enough you'll get away");
                                                	scanf("%d", &choice);


                                                	if (choice == 1)
							{
                                                        	puts("Selected light armor");
                                                        	printf("HP = %d\n", lightHP);
								choice = 99;
                                                       		break;
							}
							...
							{
                                                        	puts("Selected medium armor");
						     		printf("HP = %d\n", mediumHP);
                                                        	break;
							}
							else if(choice == 3)
							{
                                                       		 puts("Selected heavy armor");
                                                        	 printf("HP = %d",heavyHP);
                                                       		 break;
							}
                                         		break;		}
							...
							else if(choice == 3)
							{
								...
								int totalcops;
								int copHP = 90;
								int playerHP = (carselected + armorselected);
								int numCops = 1 + rand()%10;

								int lostby;
								int wonby;
								lostby = totalcops - playerHP;
								wonby = playerHP - totalcops;

								totalcops = copHP * numCops;
					
								printf("We got %d points to make it out\n", playerHP);
								printf("Total Cops %d, HP points %d\n", numCops, totalcops);	

								if(totalcops > playerHP)
								{
									puts("Oh no looks like we are outnumbered");
									puts("Game over!");
									puts("Quit and type 'cat stats.txt' to see your stats");
									fprintf(wptr,"You lost by %d points\n",lostby);

						
								}
								if (totalcops < playerHP)
								{
									puts("Nice we got away!");
									puts("Enter You Name");
									scanf("%s", &name);
									printf("Thank you for playing %s\n",(name));
									puts("Quit and type 'cat stats.txt' to see your stats");
									fprintf(wptr,"%s won by %d points\n",name,wonby);
